...
- **Documented Gilgamesch-vs-DipNet divergences (class b)** — RESOLVED-AS-SWITCHED (2026-09-10): the three divergence families (VIA-flagged no-con stand; unflagged adjacent + con ignores convoy; bounced convoyed attack cut) are now governed by `Switches.convoy_via_explicit` (default False = Dippy semantics; 1000-game DipNet 16790/86 → **16809/67**, the 19 class-b FAILs closed). The Gilgamesch readings remain available with the switch on. Also in this change: dead-route/ambiguous convoy stands report `succeeds=False` (was None), and geo-INVALID umoves (`$uinv`) no longer cut supports (B.4.2.9, closes the 3 succeeds-regressions found in triage). DipNet 100-game gate: 1600/2 (only the class-c con-dest + con-reporting wire families remain); full 33k-game corpus 584271/3358 (99.4%). See `project/doc/RULE_SWITCHES.md`.

## Implementation Notes (stable)

- Conflict resolver remains field-name agnostic and should receive normalized superfields.
- **Conflicter is geography/switch-blind (2026-09-10):** convoy moves reach `conflict_game` as `OrderType.cmve` (new enum member); the mve-vs-cmve decision is made by the order pre-processor (`dipworkpy/order_prep.py`, classification core `geography.convoy.classify_cmove_candidates`) per the `convoy_via_explicit` switch (default False = Dippy/DipNet: flag informational when a land route exists, con order claims the move → adjacent swap works; flagged move without land route = convoy attempt → `$criv` full-strength stand). The conflict parser no longer reads `Order.via_convoy` or the switch. See `project/doc/RULE_SWITCHES.md`. Planned: subfield→superfield normalization moves into the pre-processor too.
