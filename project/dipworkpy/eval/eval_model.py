...
class t_field(BaseModel):
    ...
    patt: bool = False  # movement-phase standoff (C.2.2/C.2.3.1); eval marks, writer collects (Task 10)
    # logging
    _events: List[str] = []
    ...
    def __log__(self):
        ...
        return (
            f"{self.player} {self.strength} {self.name} {self.order.name} {self.dest}@{self.xref}"
