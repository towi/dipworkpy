...
The 14 examples in [EXAMPLES.md](EXAMPLES.md) cover holds, moves, bounces, supports, convoys, dislodgement, pattfields, subfield resolution, and the B.4.2.9 / B.4.2.10 invalid-order asymmetry. The diagrams use an [orthogonal visual system](examples/README.md#orthogonal-visual-conventions) — shape encodes the order type, line style encodes success vs. failure, color encodes the nation. Each axis is independent.
